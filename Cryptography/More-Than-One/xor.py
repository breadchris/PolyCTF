...
def encrypt(data, key):
    out = ""
    for char, key in zip(data, key * (len(data) / len(key))):
        out += chr(ord(char) ^ ord(key))
    return out
...
def main(argv):
    ...
    if argv[1] == "decrypt":
        print "[*] Decrypting file with xor key:", list(xor_key)
        print decrypt(file_data.decode("hex"), xor_key)
        print "[+] Decryption Finished"
    elif argv[1] == "encrypt":
        print "[*] Encrypting file with xor key:", list(xor_key)
